...
```mermaid
flowchart LR
    Visitante((Visitante)) --> UC3(Conocer propósito del sitio)
    
    UC3 -. include .-> UC3_1(Leer sección Qué es CaliLand)
    UC3 -. include .-> UC3_2(Ver explicación de experiencia introductoria)
    UC3 -. include .-> UC3_3(Identificar zonas culturales y turísticas)
    UC3 -. extend .-> UC3_4(Navegar a la exploración)
